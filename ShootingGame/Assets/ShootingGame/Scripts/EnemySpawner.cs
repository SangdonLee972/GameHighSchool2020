...
public class EnemySpawner : MonoBehaviour
{
    ...
    public float m_SpawnIntervaiMax = 6f;
    ...
    private void Start()
    {
        m_SpawnCooldown = Random.Range(m_SpawnIntervalMin, m_SpawnIntervaiMax);
    }
    void Update()
    {
        if (m_SpawnCooldown <= 0)
        {
            int SpawncCout = Random.Range(m_MinSpawnCount, m_MaxSpawnCount);

            List<int> spawnNums = new List<int>();
            for (int i = 0; i < SpawncCout; i++)
            {
                int spawnNum;
                do
                {
            ...
            foreach (var spawnNum in spawnNums)
            {
                ...
                GameObject bullet = GameObject.Instantiate(m_EnemyPrefab,
                    m_SpawnPoints[spawnNum].position, Quaternion.Euler(eulerAngle)); //불렛생성
            }
            m_SpawnCooldown = Random.Range(m_SpawnIntervalMin, m_SpawnIntervaiMax);
        }
